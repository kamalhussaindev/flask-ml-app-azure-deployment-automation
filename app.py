...
path_wkhtmltopdf = os.environ.get('WKHTMLTOPDF_PATH', '/usr/bin/wkhtmltopdf')
...
app.config['MYSQL_DB'] = os.environ.get('MYSQL_DB', 'epca')
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'
# File uploads (use relative Linux paths inside container)
...
# Lazy-load model to avoid high memory use during worker boot.
model = None
...
def get_model():
    ...
    if model is None:
        ...
        candidate_paths = [
            os.path.join('static', 'models', 'VGG16_for_Breast.h5'),
            os.path.join('static', 'models', 'VGG16.h5'),
        ]
        model_path = next((path for path in candidate_paths if os.path.exists(path)), None)
        if model_path is None:
            raise FileNotFoundError('No model file found in static/models')
...
def predict_label(img_path):
    ...
    image = Image.open(img_path).convert('RGB').resize((50, 50))
    i = np.asarray(image, dtype=np.float32) / 255.0
    i = i.reshape(1, 50, 50, 3)  # reshape to (1,50,50,3)
    result = loaded_model.predict(i, verbose=0)
...
#              # Reading, resizing, saving and preprocessing image for predicition
              ...
              try:
                  ...
                  app.logger.exception('Image classification failed for %s', img_path)
                  flash('Classification failed. Please try again with another image.','danger')
                  return render_template('classify_image.html', full_filename=full_filename)
